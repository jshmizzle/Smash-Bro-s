...
public class MainGamePanel extends JPanel {
	...
	private Image boulder, megaman, sonic, grass, mario, goku, link, princess, waypoint, tree, invalidMove, chestClosed; 
...
//		this.currentUnit=gameBoard.getUserUnits().get(1);
	...
	public void initializeImages(){
		try {
			...
			chestClosed=ImageIO.read(new File("images/chestClosed.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
		...
		for(int row=0; row<currentBoard.length; row++){
			for(int col=0; col<currentBoard[0].length; col++){
				if(currentBoard[row][col]==' '){
					...
					g2.drawImage(grass, col*gameTileWidth, row*gameTileHeight, gameTileWidth, gameTileHeight, null);
					g2.drawImage(chestClosed, col*gameTileWidth, row*gameTileHeight, gameTileWidth, gameTileHeight, null);
				}
			}
		}
		drawCursor(g2);
